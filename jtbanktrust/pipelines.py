...
class JjtbanktrustPipeline:

    # Database setup
    conn = sqlite3.connect('jtbanktrust.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `jtbanktrust`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM jtbanktrust WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO jtbanktrust (date, title, link, content)"
